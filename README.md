...
| 35 | [economic-setup.md](35-economic-setup.md) | **Economic Setup deep-read:** 12 sub-sections covering the full pricing chain (Currencies, Taxes + Tax Groups, Departments, 24-field Price Keys, Modifiers, Categories, Packages + Open Choice Menus, Automatic + Manual Discounts, Denominations, Fast Sale Items, Lane Orders with BES X score-console flow). The most heavily-touched Setup module for a Kings-style venue. |
| 36 | [mms-chm-read.md](36-mms-chm-read.md) | **MMS operator-facing config deep-read:** 5 sub-sections (Monitor Management with 6 session types, Sequence Setup for timed rotations, Strike Challenge cross-lane competition, Monitor Setup hardware registration, Program Upload). Complements doc 18 (which documents the Node.js delivery layer) by covering the Conqueror UI config surface. |
| 37 | [appendix.md](37-appendix.md) | **Appendix (Quick Access Menu Icons) + CHM coverage checkpoint.** Short doc closing out the CHM read: 13 unlabeled icon images, pointers to the raw HTML for visual reference, and a full checklist showing every one of the 30 CHM top-level sections has been covered by the numbered doc series. |
